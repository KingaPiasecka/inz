...
public class PM1MapInterface implements MapInterface {

    private final Context context;

    public PM1MapInterface(Context context) {
        this.context = context;
    }

    @Override
    public MarkerOptions getMeasurementMarker(Dust dust) {
        BitmapDescriptor bitmapDescriptor = getMeasurementDescriptor(dust.getPm1());

        LatLng pos = new LatLng(dust.getLatitude(), dust.getLongitude());
        MarkerOptions markerOptions = new MarkerOptions()
                .position(pos)
                .title("PM1: " + dust.getPm1().toString())
